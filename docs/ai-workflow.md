...
## Agent guidance documentation PR

The candidate asked whether to add `AGENTS.md`, then requested a PR and explicitly approved the plan to add root guidance, link it from the README, and restore this missing workflow record. Codex restored the original record from the implementation branch and added guidance for component boundaries, event identity, leases, resource bounds, validation, and Docker Desktop Kubernetes compatibility.

Validation for this documentation-only change checks local Markdown links, referenced paths, Makefile targets, and whitespace with `git diff --check`. Application code and previously measured results are unchanged; application tests are not rerun for this PR. No human code intervention is claimed for this addition.
